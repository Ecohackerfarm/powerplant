...
function assertCombinations(actual, expected) {
	const equal = actual.every(actualCombination => expected.some(expectedCombination => areEqualCombinations(actualCombination, expectedCombination)));
	assert.equal(equal, true, 'Sets of combinations of size ' + actual[0].length + ' are not equal');
}
...
describe('Combinations', () => {
	it('generates correct combinations', () => {
		...
		assertCombinations([[0, 1], [0, 2], [0, 3], [1, 2], [1, 3], [2, 3]], combinations.getCombinations(2));
		assertCombinations([[0, 1, 2], [0, 1, 3], [0, 2, 3], [1, 2, 3]], combinations.getCombinations(3));
	});
	it('removes all combinations that contain the removed element', () => {
		...
		assertCombinations([[1, 2], [1, 3], [2, 3]], combinations.getCombinations(2));

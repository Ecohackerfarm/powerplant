...
export function areEqualCombinations(combination0, combination1) {
	return combination0.every(element => combinationContainsElement(combination1, element));
}
...
export class Combinations {
		...
		for (;;) {
			...
			this.combinations[combinationSize].forEach(combination => {
				elements.forEach(element => {
					...
					if ((!combination.includes(element)) && combination.every(combinationElement => combinationElement.isCompatible(element))) {
						let newCombination = combination.concat([element]);
						// Check if the new combination is unique
						if ((newCombinations.length == 0) || newCombinations.every(anotherNewCombination => (!areEqualCombinations(anotherNewCombination, newCombination)))) {
	...
	removeElement(element) {
		for (let size = 1; size <= this.getLargestCombinationSize(); size++) {
			this.combinations[size] = this.combinations[size].filter(combination => (!combinationContainsElement(combination, element)));
		}
	}

			...
			Promise.all(promises).then(data => {
				...
				for (let crop1 in companionships) {
					for (let crop2 in companionships[crop1]) {
						const compatibility =
							companionships[crop1][crop2] === 'good' ? 1 : -1;
						savePromises.push(
							new Companionship({
								crop1: fbToMongo[crop1],
								crop2: fbToMongo[crop2],
